...
from IMS.user_forms import FacultyForm, FacultyFormModify
    ...
    if request.method == 'GET':
        if 'term' in request.GET:
            ...
            if not searchTerm:
                ...
                if searchType == 'id':
                    courses = Faculty_user.objects.filter(id=searchTerm)
                elif searchType == 'name':
                    courses = Faculty_user.objects.filter(name__icontains=searchTerm)
                # else:
        ...
        if 'deleteid' in request.POST:
            ...
            if 'deleteTerm' in request.COOKIES and 'deleteType' in request.COOKIES:
                ...
                if searchType == 'faculty_id':
                    courses = Faculty_user.objects.filter(id=searchTerm)
                elif searchType == 'faculty_name':
                    courses = Faculty_user.objects.filter(name__icontains=searchTerm)
            response = render(request, 'DeleteFaculty.html', locals())
    return response
